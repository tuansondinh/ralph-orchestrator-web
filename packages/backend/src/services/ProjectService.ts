...
function resolveDefaultRalphBackend(): LoopBackend {
  const envValue = process.env.RALPH_UI_DEFAULT_BACKEND
  if (typeof envValue === 'string') {
    const fromEnv = asLoopBackend(envValue.trim().toLowerCase())
    if (fromEnv) {
      return fromEnv
    }
  }
...
function buildDefaultRalphConfigContent(defaultBackend: LoopBackend) {
  return `# Ralph Orchestrator Configuration
# Generated by: ralph init --backend ${defaultBackend}
...
async function ensureProjectRalphConfig(path: string) {
  ...
  await writeFile(configPath, buildDefaultRalphConfigContent(resolveDefaultRalphBackend()), 'utf8')
