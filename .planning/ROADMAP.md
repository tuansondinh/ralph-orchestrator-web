...
This roadmap delivers cloud integration for Ralph Orchestrator in 7 phases, progressing from database foundation through authentication, multi-tenancy, remote execution, supporting services, advanced features, and finally cloud UI with infrastructure. Each phase delivers a coherent capability that unblocks the next. Local mode remains untouched throughout.
...
**Requirements**: DB-01, DB-02, DB-03, DB-04, DB-05, MODE-01, MODE-02, MODE-03
**Success Criteria** (what must be TRUE):
  1. App starts in local mode with SQLite when no cloud env vars are set, behaving identically to today
  2. App starts in cloud mode with Supabase Postgres when SUPABASE_URL and related env vars are configured
  3. All existing features (loops, chat, terminals, projects) work identically against both database backends
  4. Feature detection API reports available capabilities based on current mode
**Plans**: TBD
...
**Depends on**: Phase 2
**Requirements**: ORG-01, ORG-02, ORG-03, ORG-04, ORG-05, PROJ-01, PROJ-02, PROJ-03, PROJ-04
**Success Criteria** (what must be TRUE):
...
**Depends on**: Phase 3
**Requirements**: RUN-01, RUN-02, RUN-03, RUN-04, RUN-05, RUN-06, SESS-01, SESS-02, SESS-03, SESS-04
**Success Criteria** (what must be TRUE):
...
**Depends on**: Phase 3
**Requirements**: CRED-01, CRED-02, CRED-03, CRED-04, USE-01, USE-02, USE-03, AUD-01
**Success Criteria** (what must be TRUE):
...
**Depends on**: Phase 4
**Requirements**: PREV-01, PREV-02, PREV-03, PREV-04, SRC-01, SRC-02, SRC-03, RT-01, RT-02, RT-03
**Success Criteria** (what must be TRUE):
...
**Goal**: Complete cloud frontend experience and one-command infrastructure deployment
**Depends on**: Phase 5, Phase 6
**Requirements**: UI-01, UI-02, UI-03, UI-04, UI-05, UI-06, UI-07, UI-08, UI-09, INFRA-01, INFRA-02, INFRA-03
**Success Criteria** (what must be TRUE):
  1. All cloud features have dedicated UI pages matching existing app component patterns
  2. Login/signup, org management, credential settings, usage dashboard, remote monitoring, preview management, and collaborator pages are functional
  3. CDK stacks deploy VPC, ECS cluster, and SQS queues with a single command
  4. Mode indicator in UI shows whether app is running in local or cloud mode
**Plans**: TBD
