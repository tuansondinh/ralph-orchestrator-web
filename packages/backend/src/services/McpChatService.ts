...
export class McpChatService {
  ...
  async streamChat(input: McpChatStreamInput) {
    ...
    const stream = this.streamText({
      ...
      tools: this.buildToolsMap(input.sessionId, toolLinksByCallId),
  ...
  private buildToolsMap(sessionId: string, toolLinksByCallId: Map<string, string>) {
      ...
      tool<unknown, string>({
        ...
        execute: async (args: unknown, options: { toolCallId: string }) => {
          ...
          if (requiresConfirmation && options.toolCallId) {
            const confirmed = await this.getOrCreatePendingConfirmation(
              sessionId,
              options.toolCallId
            ).promise
