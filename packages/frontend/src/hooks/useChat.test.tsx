...
describe('useChat', () => {
  ...
  it('posts chat stream input and applies text/tool events to store', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:3003/trpc/chat/stream',
  ...
  it('posts tool confirmation decisions and updates confirmation status', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(fetchMock).toHaveBeenCalledWith(
      'http://127.0.0.1:3003/trpc/chat/confirm',
      expect.objectContaining({
        method: 'POST'
      })
    )
  ...
  it('finalizes stream state when stream closes without done/error event', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(
        createSseStream([
          {
  ...
  it('clears stale pending confirmation when stream closes without terminal event', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(
        createSseStream([
          {
