...
describe('EC2 deployment artifacts', () => {
  ...
  it('ships docker artifacts with compose startup and opencode wiring', () => {
    ...
    const opencodeConfig = readDeployFile('opencode.json')
    ...
    expect(entrypoint).toContain("const templatePath = '/app/deploy/opencode.json'")
    expect(entrypoint).toContain('url: `http://127.0.0.1:${port}/mcp`')
    expect(opencodeConfig).toContain('"mcp"')
    expect(opencodeConfig).toContain('"ralph"')
  ...
  it('ships a syntactically valid deploy script with build, migrate, and restart steps', () => {
    ...
    expect(deployScript).toContain("url: `http://127.0.0.1:${port}/mcp`")
