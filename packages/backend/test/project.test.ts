...
describe('project tRPC routes', () => {
  ...
  it('uses RALPH_UI_DEFAULT_BACKEND when scaffolding a default ralph.yml', async () => {
    const caller = await createCaller()
    const projectDir = await createTempDir('project-default-backend-env')
    ...
    try {
      await caller.project.create({
        name: 'Backend From Env',
        path: projectDir
      })

      const generated = await readFile(join(projectDir, 'ralph.yml'), 'utf8')
      expect(generated).toContain('Generated by: ralph init --backend opencode')
      expect(generated).toContain('backend: "opencode"')
    } finally {
